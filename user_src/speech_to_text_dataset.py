...
from .audio_utils import get_features_or_waveform
...
class SpeechToTextDataset(FairseqDataset):
    ...
    def get_src_lens_and_check_oov(self):
        if self.src_texts == self.n_samples * ['']:
    ...
    def collater(
        ...
        target_src, target_src_lengths = None, None
        prev_output_src_tokens = None
        if self.src_texts != self.n_samples * ['']:
            target_src = fairseq_data_utils.collate_tokens(
                [x.source for x in samples],
                self.tgt_dict.pad(),
                self.tgt_dict.eos(),
                left_pad=False,
            ...
            target_src = target_src.index_select(0, order)
            target_src_lengths = torch.tensor(
                [x.source.size(0) for x in samples], dtype=torch.long
            ).index_select(0, order)
        ...
        out = {
            ...
            "target_src_lengths": target_src_lengths,
            "ntokens": ntokens,
